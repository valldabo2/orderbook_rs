...
fn test_speed(){
    ...
    for i in 0..n_orders {
        let p = OrderedFloat(
            rng.gen_range(price_range.clone()) as f64
        );
        let s = rng.gen_range(size_range.clone()) as f64;
        let side = sides.choose(&mut rng).unwrap();
        orders.push_back(Order{price: p, size: s, side: *side, id: i});
    }
    ...
    println!("Orders:{} in {:?}, orders/sec: {:?}", n_orders, duration, orders_per_second);
